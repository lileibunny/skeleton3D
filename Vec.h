...
#define CROSS %
	...
	template <int D, class T>
	static inline Vec<D, T> operator * (const Vec<D, T> &v, double &x)
	{
		Vec<D, T> result(VEC_UNINITIALIZED);
		for (int i = 0; i < D; i++)
			result[i] = v[i] * x;
...
#undef VEC_DECLARE_THREEARG
	...
	typedef Vec<3, float> Point3f;
	typedef Vec<3, int> Point3i;
#endif
